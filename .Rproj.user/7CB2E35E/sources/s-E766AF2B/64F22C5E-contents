...
#' More description
#'
#' @param x a data frame with results of the gene set over-representation analysis.
#' Must have a rownames indicating names of the instances and at least two columns,
#' one of which contains gene members; and another one that contains p-values
#' (raw or adjusted for multiple testing).
...
#' @param scale.factor a positive real number to control dependence of the resulting circle radius on the number of gene members of the given instance.
...
#' @param isomap.k a positive integer indicating number of k nearest neighbors of
#' the \emph{isomap} projection.
#' @param tsne.perplexity a positive integer indicating \emph{tSNE} perplexity.
#' @param tsne.iterations a positive integer indicating maximum number of \emphPtSNE} iterations to perform.
#' @param cca.epochs a positive integer indicating \emph{CCA} training length.
#' @param no.clusters a positive integer indicating number of clusters, must be less than number of instances.
#' @param max.clusters a positive integer indicating maximum number of clusters to consider, must be at least two.
#' @param cluster.stat a character indicating statistic used to select optimal number of clusters.
...
#' @param pam.boots a positive integer indicating number of boostrap used to be used when selecting number of clusters.
#'
...
#'     the number of gene members; instance weight (-log10(p-value)), centrality
#'     and cluster membership.
#'     \item \code{stress} Kruskall stress caused by projection and circle packing.
#'     \item \code{spcorr} Spearman correlation between the original instance distances/dissimilarities
#'     and those obtained after projection and circle packing.
#' }
#'
